...
def get_response_of_llm(chat_dest: str, user_msg: str):
    results = controller_embeddings.get_silimitary(user_msg)
    message_to_llm = f"""
    Sabiendo que eres un agente de atención al cliente, queremos que generes una respuesta completa y personalizada para la consulta del cliente. Utiliza un tono amigable y natural, adaptado a un público joven de entre 18 y 30 años. Asegúrate de que la respuesta sea clara, directa y no muy extensa, manteniéndola alrededor de 200 caracteres. También incluye una breve sugerencia sobre los siguientes pasos a seguir en la conversación, de forma que fluya de manera natural.
    Por favor, transforma esta información en una respuesta concisa, usando un lenguaje cercano y fácil de entender.
    Ademas te adjunto los mensajes previos asi que usalos para contruir un flujo de conversacion y asi mejorar la respuesta del cliente, no repitas informacion obvia que ya dijiste en la conversacion evita deciir palabras que inicien la conversacion como Hola etc.

    Es muy importante que sea un lenguaje juvenil
    ...
    Esto es indispensable: si no tienes el nombre de el usuario en la conversacion o si el prev_message es None, solicitale su nombre
...
async def echo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if audio:
        ...
        await update.message.reply_voice(voice=audio.file_id)
    if text:
        response = get_response_of_llm(chat_dest=update.message.author_signature, user_msg=text)
...
def main() -> None:
    ...
    application = (
        Application.builder()
        .token("8029973101:AAEdYkYyuI14dpW2SacHwrqMuXqqd55vM0I")
        .build()
    )
